...
@app.post("/api/v1/announcements", response_model=Announcement, status_code=201)
async def create_announcement(announcement: Announcement):
    try:
        Database.id_counter += 1  # Increment the counter
        announcement.id = Database.id_counter  # Assign the generated ID
...
@app.put("/api/v1/announcements/{announcement_id}", response_model=Announcement)
async def update_announcement(announcement_id: int, announcement: Announcement):
    try:
        if announcement_id < 1 or announcement_id > Database.id_counter:
            raise HTTPException(status_code=404, detail="Announcement not found")
        if announcement.id != announcement_id:
            raise HTTPException(status_code=400, detail="Cannot change announcement ID")
